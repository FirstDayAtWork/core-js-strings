...
function removeFirstOccurrences(str, value) {
  return str.indexOf(value) < 0 ? str : str.replace(value, '');
}
...
function sumOfCodes(str) {
  return !str ? 0 : str.split``.reduce((a, b) => a + +b.charCodeAt(0), 0);
}
...
function isPalindrome(str) {
  const replacer = str.toLowerCase().replace(/[^\w]/g, '');
  const reverser = replacer.split``.reverse().join``;
  return replacer === reverser;
}
...
function findLongestWord(sentence) {
  ...
  const len = arr.map((el) => el.length);
  const max = Math.max(...len);
  const index = len.indexOf(max);
  return arr[index];
}
...
function encodeToRot13(str) {
  ...
  return str.replace(/\w/g, (letter) => rot13[alphabet.indexOf(letter)]);
}
